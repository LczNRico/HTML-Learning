...
const THEME_DEFAULTS = {
  mode: "light",
  appBg: "#faf7f4", topBarBg: "#f0ebe3", sidebarBg: "#ede7df",
  columnBg: "#e8e2d9", libCardBg: "#fdfaf7", slotBg: "#f5efe8",
};
...
const persist      = async (d) => { try { localStorage.setItem(STORE_KEY, JSON.stringify(d)); } catch {} };
const persistTheme = async (t) => { try { localStorage.setItem(THEME_KEY, JSON.stringify(t)); } catch {} };
const hydrate = async () => {
  try {
    const el = document.getElementById('__app_data__');
    if (el) { const d = JSON.parse(el.textContent); if (d && (d.exercises || d.days)) return d; }
    const v = localStorage.getItem(STORE_KEY); if (v) return JSON.parse(v);
  } catch {} return null;
};
const hydrateTheme = async () => {
  try {
    const el = document.getElementById('__app_theme__');
    if (el) { const t = JSON.parse(el.textContent); if (t && t.mode) return t; }
    const v = localStorage.getItem(THEME_KEY); if (v) return JSON.parse(v);
  } catch {} return null;
};
const saveToFile = (data, themeData) => {
  let html = document.documentElement.outerHTML;
  const dataTag = '<script id="__app_data__" type="application/json">';
  const themeTag = '<script id="__app_theme__" type="application/json">';
  const closeTag = '<\/script>';
  html = html.replace(new RegExp(dataTag.replace(/[<>/]/g, c => '\\' + c) + '[\\s\\S]*?' + closeTag), dataTag + JSON.stringify(data) + '<\/script>');
  html = html.replace(new RegExp(themeTag.replace(/[<>/]/g, c => '\\' + c) + '[\\s\\S]*?' + closeTag), themeTag + JSON.stringify(themeData) + '<\/script>');
  const blob = new Blob(['<!DOCTYPE html>\n' + html], { type: 'text/html;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href = url; a.download = 'training-planner.html';
...
export default function App() {
  ...
  const [tab, setTab]                 = useState("plan"); // "plan" | "schedule"
  const [scheduleEvents, setSchedEvs] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (d) { setExercises(d.exercises||[]); setDays(d.days||[newDay(0)]); setPlanName(d.planName||"我的訓練計畫"); setSchedEvs(d.scheduleEvents||[]); }
      if (t) setTheme({ ...THEME_DEFAULTS, ...t });
      setLoaded(true);
    })();
  }, []);

  useEffect(() => { if (loaded) persist({ exercises, days, planName, scheduleEvents }); }, [exercises, days, planName, scheduleEvents, loaded]);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden transition-colors"
      ...
      <div className="shrink-0 flex items-center gap-4 px-4 py-3 z-20"
        ...
        {/* Tab switcher */}
        <div className="flex gap-1 rounded-xl p-1" style={{ backgroundColor: p.btnBg }}>
          {[["plan","訓練計畫"],["schedule","週時間表"]].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)}
              className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
              style={{
                backgroundColor: tab === id ? accentBtn : "transparent",
                color: tab === id ? "#fff" : p.text3,
              }}>
              {label}
            </button>
          ))}
        </div>
        {/* Plan name — only visible in plan tab */}
        {tab === "plan" && (
          <input value={planName} onChange={e => setPlanName(e.target.value)}
            className="flex-1 bg-transparent font-black text-base outline-none min-w-0"
            style={{ color: p.text1 }}
            placeholder="計畫名稱" />
        )}
        {tab === "schedule" && (
          <span className="flex-1 font-black text-base" style={{ color: p.text2 }}>一週行程表</span>
        )}
        <div className="flex items-center gap-2 shrink-0">
          <div className="text-xs hidden sm:block" style={{ color: p.text4 }}>{days.length} 天 · {totalSlots} 個動作</div>

          {/* Day adjuster — plan tab only */}
          {tab === "plan" && <div className="flex items-center rounded-xl px-2 py-1 gap-1" style={{ backgroundColor: p.btnBg }}>
          ...
          <button onClick={async () => {
            const data = { exercises, days, planName, scheduleEvents };
            await persist(data); await persistTheme(theme);
            saveToFile(data, theme);
...
const SCHED_CATS = [
  { id: "training",   label: "重訓",   color: "#ef4444" },
  { id: "basketball", label: "籃球",   color: "#f97316" },
  { id: "study",      label: "讀書",   color: "#3b82f6" },
  { id: "meal",       label: "飲食",   color: "#22c55e" },
  { id: "sleep",      label: "睡眠",   color: "#8b5cf6" },
  { id: "rest",       label: "休息",   color: "#64748b" },
  { id: "other",      label: "其他",   color: "#eab308" },
];
const DAYS_TW = ["週一","週二","週三","週四","週五","週六","週日"];
const SCHED_HOURS = Array.from({ length: 17 }, (_, i) => i + 7); // 7:00–23:00
const CELL_H = 54; // px per hour
const getSchedCat = (id) => SCHED_CATS.find(c => c.id === id) || SCHED_CATS[SCHED_CATS.length - 1];
const fmtHour = (h) => `${String(h % 24).padStart(2,"0")}:00`;

// ── Schedule Event Modal ──────────────────────────────────────────────────────

function ScheduleModal({ initial, onSave, onClose, p }) {
  const [form, setForm] = useState(initial);
  const set = (patch) => setForm(f => ({ ...f, ...patch }));

  const endOptions = Array.from({ length: 24 - form.startHour }, (_, i) => form.startHour + i + 1);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center"
      ...
      <div className="w-80 rounded-2xl p-5 shadow-2xl"
        ...
        <h2 className="text-sm font-black mb-4" style={{ color: p.text1 }}>
          新增行程 — {DAYS_TW[form.day]}
        </h2>

        {/* Title */}
        <label className="block mb-3">
          <span className="text-xs block mb-1" style={{ color: p.text3 }}>標題</span>
          <input autoFocus
            className="w-full rounded-xl px-3 py-2 text-sm outline-none"
            style={{ backgroundColor: p.inputBg, border: `1px solid ${p.border}`, color: p.text1 }}
            placeholder="例：早餐、上課、重訓…"
            value={form.title}
        ...
        <div className="flex gap-2 mb-3">
          <div className="flex-1">
            <span className="text-xs block mb-1" style={{ color: p.text3 }}>開始</span>
            <select value={form.startHour}
              onChange={e => { const h = parseInt(e.target.value); set({ startHour: h, endHour: Math.max(form.endHour, h + 1) }); }}
              className="w-full rounded-xl px-2 py-2 text-sm outline-none"
              style={{ backgroundColor: p.inputBg, border: `1px solid ${p.border}`, color: p.text1 }}>
              {SCHED_HOURS.map(h => <option key={h} value={h}>{fmtHour(h)}</option>)}
            </select>
          </div>
          <div className="flex-1">
            <span className="text-xs block mb-1" style={{ color: p.text3 }}>結束</span>
        ...
        {/* Day */}
        <div className="mb-3">
          <span className="text-xs block mb-1.5" style={{ color: p.text3 }}>星期</span>
          <div className="flex gap-1 flex-wrap">
            {DAYS_TW.map((d, i) => (
              <button key={i} onClick={() => set({ day: i })}
                className="px-2 py-1 rounded-lg text-xs font-bold transition-all"
                style={{
                  backgroundColor: form.day === i ? "#3b82f6" : p.tagBg,
                  color: form.day === i ? "#fff" : p.tagText,
                }}>
                {d}
              </button>
            ))}
        ...
        {/* Category */}
        <div className="mb-5">
          <span className="text-xs block mb-1.5" style={{ color: p.text3 }}>類別</span>
          <div className="flex gap-1 flex-wrap">
            {SCHED_CATS.map(cat => (
              <button key={cat.id} onClick={() => set({ category: cat.id })}
                className="px-2 py-1 rounded-lg text-xs font-bold transition-all"
                style={{
                  backgroundColor: form.category === cat.id ? cat.color : cat.color + "22",
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => { onSave({ ...form, id: uid() }); onClose(); }}
            className="flex-1 py-2 rounded-xl text-sm font-bold text-white disabled:opacity-40"
...
function WeekSchedule({ scheduleEvents, setScheduleEvents, p, theme }) {
  const [showModal, setShowModal]   = useState(false);
  ...
  const openAdd = (day, hour) => {
    setDraft({ day, startHour: hour, endHour: hour + 1, title: "", category: "other" });
    setEditingEv(null);
    setShowModal(true);
  };
  ...
  const saveEvent = (ev) => {
    if (editingEv) {
      setScheduleEvents(prev => prev.map(e => e.id === ev.id ? ev : e));
    } else {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ backgroundColor: theme.appBg }}>
        ...
        {DAYS_TW.map((d, i) => (
          <div key={i} className="flex-1 text-center py-2.5"
            ...
            <div className="text-sm font-black"
              style={{ color: isWeekend(i) ? "#ef4444" : p.text1 }}>{d}</div>
            <div className="text-xs mt-0.5" style={{ color: p.text4 }}>
              {scheduleEvents.filter(ev => ev.day === i).length > 0
                ? `${scheduleEvents.filter(ev => ev.day === i).length} 項`
                : ""}
            </div>
          </div>
        ))}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-auto">
        <div className="flex" style={{ minWidth: "540px", position: "relative" }}>

          {/* Time labels column */}
          <div className="shrink-0" style={{ width: "52px", backgroundColor: theme.topBarBg }}>
            {SCHED_HOURS.map(h => (
              <div key={h} className="flex items-center justify-end pr-2 shrink-0"
                style={{ height: `${CELL_H}px`, borderTop: `1px solid ${p.border}` }}>
                <span className="text-xs font-mono" style={{ color: p.text3 }}>
                  {fmtHour(h)}
                </span>
              </div>
            ))}
            ...
            <div className="flex items-center justify-end pr-2"
              style={{ height: "20px" }}>
              <span className="text-xs font-mono" style={{ color: p.text3 }}>00:00</span>
            </div>
          </div>

          {/* Day columns */}
          {DAYS_TW.map((_, dayIdx) => {
            const dayEvents = scheduleEvents.filter(ev => ev.day === dayIdx);
            return (
              <div key={dayIdx} className="flex-1 relative"
                style={{ borderLeft: `1px solid ${p.border}`, minHeight: `${CELL_H * 17}px` }}>

                {/* Hour cells — click targets */}
                {SCHED_HOURS.map(h => (
                  <div key={h}
                    onClick={() => openAdd(dayIdx, h)}
                    onMouseEnter={() => setHoverCell({ day: dayIdx, hour: h })}
                    onMouseLeave={() => setHoverCell(null)}
                    style={{
                      height: `${CELL_H}px`,
                      borderTop: `1px solid ${p.border}`,
                      cursor: "pointer",
                      backgroundColor: hoverCell?.day === dayIdx && hoverCell?.hour === h
                        ? (theme.mode === "light" ? "rgba(0,0,0,0.04)" : "rgba(255,255,255,0.03)")
                        : "transparent",
                ...
                {/* Event blocks */}
                {dayEvents.map(ev => {
                  const cat = getSchedCat(ev.category);
                  const top    = (ev.startHour - 7) * CELL_H;
                  const height = (ev.endHour - ev.startHour) * CELL_H;
                  return (
                    <div key={ev.id}
                      ...
                      <div className="px-1.5 py-1 h-full flex flex-col justify-between">
                        <div>
                          <div className="text-xs font-bold truncate leading-tight"
                            style={{ color: cat.color }}>{ev.title}</div>
                          {height >= 44 && (
                        ...
                        {height >= 60 && (
                          <button
                            ...
                            className="self-end opacity-0 group-hover:opacity-100 text-xs px-1.5 py-0.5 rounded-md transition-opacity"
      ...
      {showModal && draft && (
        <ScheduleModal
          initial={draft}
          onSave={ev => { saveEvent(ev); setShowModal(false); setEditingEv(null); }}
          onClose={() => { setShowModal(false); setEditingEv(null); }}
          p={p}
        />
      )}
    </div>
